...
function normalizePacket(p,diagnostic={}){
 ...
 if(!q&&![result.a,result.g,result.m].some(v=>vec(v,3))){diagnostic.error=(warnings.length?warnings.join('; '):'Нет ориентации или полного вектора датчика: нужны q / rpy_deg либо ax,ay,az / gx,gy,gz / mx,my,mz')+'. Поля JSON: '+(Object.keys(p).slice(0,30).join(', ')||'(пусто)');return null}
 return result;
}
...
function drawCharts(){const history=state.paused?state.freezeHistory:state.history,end=history.at(-1)?.t??0,start=end-state.window,samples=history.filter(p=>p.t>=start),mode=state.chartMode;for(let idx=0;idx<3;idx++){const{ctx,w,h}=canvasSize($('chart-'+idx));if(!w||!h)continue;ctx.clearRect(0,0,w,h);const pad={l:46,r:13,t:15,b:26},pw=w-pad.l-pad.r,ph=h-pad.t-pad.b;const key=['a','g','m'][idx],channels=mode==='sensors'?3:1,values=samples.flatMap(p=>mode==='sensors'?p[key]:[p.rpy[idx]]).filter(finite);let lo=values.length?Math.min(...values):0,hi=values.length?Math.max(...values):1;const minSpan=mode==='sensors'?[.15,4,6][idx]:6;if(hi-lo<minSpan){const mid=(hi+lo)/2;lo=mid-minSpan/2;hi=mid+minSpan/2}const ext=(hi-lo)*.14;lo-=ext;hi+=ext;const xp=t=>pad.l+(t-start)/state.window*pw,yp=v=>pad.t+(hi-v)/(hi-lo)*ph;
 ctx.font='12px ui-monospace,monospace';ctx.textAlign='right';for(let i=0;i<4;i++){const value=lo+(hi-lo)*i/3,y=yp(value);ctx.strokeStyle='#2b3630';ctx.lineWidth=1;ctx.beginPath();ctx.moveTo(pad.l,y);ctx.lineTo(w-pad.r,y);ctx.stroke();ctx.fillStyle='#98aaa0';ctx.fillText(format(value,hi-lo<5?2:1),pad.l-7,y+4)}for(let i=0;i<=4;i++){const x=pad.l+pw*i/4;ctx.textAlign=i===0?'left':i===4?'right':'center';ctx.fillStyle='#8b9d92';ctx.fillText(i===4?(state.mode==='log'?'конец':'сейчас'):format(-state.window*(1-i/4),state.window===10?1:0)+' с',x,h-7)}
 ctx.save();ctx.beginPath();ctx.rect(pad.l,pad.t,pw,ph);ctx.clip();for(let j=0;j<channels;j++){ctx.beginPath();let prev=null;for(const p of samples){const v=mode==='sensors'?p[key][j]:p.rpy[idx];if(!finite(v)){prev=null;continue}const x=xp(p.t),y=yp(v);if(!prev||p.t-prev.t>Math.max(.5,state.window/15)||(mode==='attitude'&&Math.abs(v-prev.v)>180))ctx.moveTo(x,y);else if(mode==='sensors'&&idx===2){ctx.lineTo(x,yp(prev.v));ctx.lineTo(x,y)}else ctx.lineTo(x,y);prev={t:p.t,v}}ctx.lineWidth=1.7;ctx.strokeStyle=COLORS[mode==='sensors'?j:idx];ctx.stroke()}ctx.restore();const p=state.display;$('chart-value-'+idx).textContent=p?mode==='sensors'?(vec(p[key],3)?'|v| '+format(norm(p[key]),2):'нет данных'):format(p.rpy[idx],2)+'°':'—';if(!values.length){ctx.textAlign='center';ctx.fillStyle='#98aaa0';ctx.fillText('Ожидание данных',w/2,h/2)}}}
